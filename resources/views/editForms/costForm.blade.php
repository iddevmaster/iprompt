...
<body>
    ...
    <form id="myForm" action="{{ route('update') }}" method="POST" >
        ...
        <div class="a4-container border mb-5 d-flex align-items-center flex-column">
            ...
            @php
                $type = explode('.', $form->type);
            @endphp
            <input type="hidden" name="formtype" id="formtype" value="{{$type[0]}}">
            ...
            <div >
                <select class="form-select" aria-label="Default select example" name="subtype">
                    ...
                    @foreach ($types as $type)
                        @if (strpos($form->type, $type->subtype) !== false)
                            <option value="{{$type->subtype}}" selected>{{$type->subtype}}</option>
                        @else
