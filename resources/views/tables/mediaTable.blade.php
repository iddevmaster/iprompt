                        ...
                        @foreach ($gendoc as $row)
                            ...
                            <tr>
                                ...
                                @if ((((App\Models\department::find((Auth::user())->dpm))->prefix) == $row->dpm || Auth::user()->hasRole(['admin', 'ceo'])) || (auth()->user()->can('download')) || (in_array((Auth::user())->dpm, $shares)) || (in_array(((Auth::user())->id), (is_array($teams)? $teams : []))))
                                    <td>
                                        <a href="{{url('/form/downloadmedia/download/'.$encryptedId)}}" target="_blank"><button type="button" class="btn btn-primary">Download</button></a>
                                    </td>
                                @else
                                    <td>
                                        <a href="{{url('/form/downloadmedia/view/'.$encryptedId)}}" target="_blank"><button type="button" class="btn btn-primary">View</button></a>
                                    </td>
                                @endif
                            ...
                            @foreach ($gendoc as $row)
                                @if (strpos($row->type, $item->subtype) !== false)
                                    ...
                                    <tr>
                                        ...
                                        @else
                                            <td>
                                                <a href="{{url('/form/downloadmedia/download/'.$encryptedId)}}" target="_blank"><button type="button" class="btn btn-primary">View</button></a>
                                            </td>
                                        @endif
