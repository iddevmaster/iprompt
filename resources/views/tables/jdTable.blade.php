...
?>
    <div class="px-lg-5 px-md-4 px-1">
        <div class="text-center mb-4"><h2>ทะเบียน มาตรฐานขั้นตอนการปฏิบัติงาน</h2></div>
        <!-- Table -->
                    ...
                    @foreach ($gendoc as $row)
                        ...
                        <tr>
                            ...
                            @if (((App\Models\department::find((Auth::user())->dpm))->prefix) == $row->dpm || Auth::user()->hasRole(['admin', 'ceo']) || (in_array((Auth::user())->dpm, $shares)))
                                <td>
                                    @if (($row->stat ?? '') !== 'ผ่านการอนุมัติ')
                                        <a href="{{url('/form/editjd/'.$row->id)}}"><button type="button" class="btn btn-warning">Edit</button></a>
                                    @endif
                                </td>
                            @else
                                <td></td>
                            ...
                            @if ((((App\Models\department::find((Auth::user())->dpm))->prefix) == $row->dpm || Auth::user()->hasRole(['admin', 'ceo'])) || (auth()->user()->can('download')) || (in_array((Auth::user())->dpm, $shares)))
                                <td>
                                    <a href="{{url('/form/downloadjd/download/'.$encryptedId)}}" target="_blank"><button type="button" class="btn btn-primary">Download</button></a>
                                </td>
                            @else
                                <td>
                                    <a href="{{url('/form/downloadjd/view/'.$encryptedId)}}" target="_blank"><button type="button" class="btn btn-primary">View</button></a>
                                </td>
                            @endif
                            ...
                            <td class="text-center">
                                ...
                                @endif
                                <button type="button" class="btn btn-info uploadBtn" value="{{$row->id}}" fileType="jd">upload</button>
                            </td>

                            {{-- Share with person --}}
                            <td><button class="btn btn-success" id="teamBtn" value="{{ $row->submit_by}}" bookid="{{ $row->id}}" bookType="jd" teamlist="{{json_encode($teamlist)}}"
                            ...
                            @can('staff')
                                <td><button class="btn btn-success" id="shareBtn" value="{{ $row->share}}" bookid="{{ $row->id}}" fileType="jd">
                                    @php
                                        $share = $row->shares;
